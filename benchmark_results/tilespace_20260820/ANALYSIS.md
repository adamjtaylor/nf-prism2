...
than 0.9. The pilot's number was a **slide-count artefact**: with 8 slides in the index there was
almost nowhere else for a tile's neighbours to be, and both of the metrics it used are bounded by
how many communities the partition has. Re-run at the pilot's slide count inside Arm A, the pilot's
numbers reappear. The confound it worried about, between-centre stain, turns out not to be the
issue at all.
...
0.381 against 0.153 chance, a lift of 2.50. PRISM2's slide embedding reaches the same lift, 2.21,
on the same task with 4,000 times less storage. And DuckDB's HNSW index, written the obvious way,
answers the filtered query with 2.8 rows and recall 0.18 without raising an error.
...
|---|---|---|---|---|---|---|---|
| Arm A, 8 slides (the pilot's count) | 8 | 13 | **0.495** | 0.324 | **43.8%** | 0.887 | 0.177 |
| Arm A, premalignant only | 23 | 42 | 0.229 | 0 by design | 8.3% | 0.494 | 0.091 |
...
draws, and it recovers the pilot's conclusion: half the variance in slide identity, and a median
community that is 89% one slide against a permuted 18%. Add the other 95 Arm A slides and the same
measurement gives 0.176 and 32%. **Holding centre, organ and protocol constant did not remove the
effect; adding slides did.** A literal reproduction of the pilot's configuration, 8 slides at 1,500
tiles each over resolutions 0.1 to 2.0, gives ARI 0.34 to 0.57 and 30 to 66% slide-dominated,
which brackets the pilot's report.
...
Arm A's ordering above Arm B and Arm C is a direct consequence of index size, 551,318 tiles against
68,427 and 47,792. The more slides in the index, the more cross-slide neighbours a tile finds,
which is the same mechanism as section 2 seen without any clustering step. It also means the
numbers here are a floor for full HTAN, not a ceiling.
...
Normal minus the rest is −0.037 (95% CI −0.061 to −0.011, bootstrapped over patients), which is
detectable and **entirely explained by specimen size rather than by class**. Among the four
...
for premalignant. In situ and primary are 16 and 15 slides of 103 and supply 90% of Arm A's
...
pilot's observation replicates exactly: it occupies 0.74 to 1.00 while base spans the full range.
...
both tasks, within overlapping intervals, and its pair orderings agree with base at Spearman 0.66,
...
progression class within one centre and organ is not, p@5 0.36 against 0.16 — and that is the same
place tile-level search lands.
...
|---|---|---|---|
| exact 1280 (scan of the unindexed column) | 472 ms | 1.000 | 1.000 |
| HNSW 1280 | 15 ms | 0.917 | 0.913 |
| exact 128 (PCA) | 29 ms | 0.785 | 0.805 |
...
variance and about 79% of the top-10 — a fair trade at a tenth of the storage and a 16-fold
speed-up, and the right default for full HTAN.
...
|---|---|---|---|---|---|
| exclude own slide | naive `WHERE ... ORDER BY ... LIMIT 10` | 9 ms | **2.8** | **93%** | **0.183** |
| exclude own slide | over-fetch 100, then filter | 4 ms | 7.8 | 27% | 0.707 |
| exclude own slide | over-fetch 1,000, then filter | 37 ms | 9.4 | 7% | **0.900** |
| exclude own slide | over-fetch 10,000, then filter | 509 ms | 9.7 | 3% | 0.970 |
| exclude own slide | exact scan | 527 ms | 10.0 | 0% | 1.000 |
| different centre only | naive | 12 ms | **0.7** | 93% | **0.003** |
| different centre only | over-fetch 100,000, then filter | 495 ms | 9.2 | 10% | 0.917 |
| different centre only | exact scan | **277 ms** | 10.0 | 0% | **1.000** |
...
1. **Over-fetch, and state the factor.** F = 1,000 gives recall 0.90 at 37 ms for slide- and
   patient-exclusion. F = 100 is 4 ms and 0.71, which is a reasonable interactive default if the
...
2. **Centre-crossing queries must be exact scans.** A tile's neighbours are overwhelmingly from its
   ...
   the exact scan is both correct and *faster*, 277 ms against 495 ms.
...
table with all three vector columns, 74 GB of 1280-d index, and an exact scan near 17 s per query.
...
instead of 667,537, and same-organ retrieval is effectively solved at slide level (p@5 0.99) while
neither level solves progression class. Against that, the tile index costs 460 GB and a 17-second
exact scan at HTAN scale, and its approximate form fails silently on exactly the filtered queries
the science requires — 2.8 rows and recall 0.18 where 10 rows and 1.00 were asked for. So: make the
...
```
python 00_build_store.py        # 163 h5 -> one memmap + tiles.parquet + PCA-128 + census.json
python 01_umap.py               # fig 1
python 02_leiden_armA.py        # fig 2, leiden_metrics.json
python 03_neighbourhood.py      # fig 3, neighbourhood_metrics.json
python 04_retrieval.py          # fig 4, retrieval_metrics.json
python 05_normalisation.py      # fig 5, normalisation_metrics.json
python 06_tile_yield.py         # fig 6, tile_yield_metrics.json, tile_yield_per_slide.csv
python 07_slide_level.py        # fig 7, slide_level_metrics.json
python duckdb/build_db.py       # analysis/data/tilesearch.duckdb  (~4 min, 15 GB)
python duckdb/bench.py          # duckdb_benchmark.md, duckdb_bench_metrics.json
python duckdb/run_examples.py   # example_queries.md
```
